...
class KingdomDashboardApp extends Application {
  ...
  get kingdom() {
    return game.actors.getName("Kingdom");
  }
  ...
  getSettlementProgressText(actor) {
    ...
    const development = this.getDevelopment(actor);
  ...
  getData() {
    ...
    const unrest = Number(kingdom.getFlag("world", "unrest") ?? 0);
    const kingdomLevel = Number(kingdom.getFlag("world", "kingdomLevel") ?? 1);
    const dc = DC_BY_LEVEL[kingdomLevel] ?? "Unknown";
  ...
  activateListeners(html) {
    ...
    html.find(".kingdom-update").on("click", async () => {
      ...
      const newDc = DC_BY_LEVEL[newLevel];
